          ...
          <div className="flex flex-wrap items-center gap-3 pt-3">
            ...
            <div className="flex items-center gap-1.5 bg-slate-100 border border-slate-200 rounded-xl px-2.5 py-1">
              ...
              <select
                ...
                className="bg-transparent border-none text-xs font-bold text-slate-700 focus:outline-none focus:ring-0 py-0.5 cursor-pointer pr-1"
              >
                {THAI_MONTHS.map((m, idx) => (
                  idx > 0 && <option key={idx} value={idx}>{m}</option>
                ))}
              </select>
              ...
              <select
                ...
                className="bg-transparent border-none text-xs font-bold text-slate-700 focus:outline-none focus:ring-0 py-0.5 cursor-pointer"
              >
                {Array.from({ length: 11 }, (_, i) => 2565 + i).map((yr) => (
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <select
                    value={selectedMonth}
                    onChange={(e) => setSelectedMonth(Number(e.target.value))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg py-1 px-2.5 text-xs font-semibold text-slate-700 focus:ring-1 focus:ring-emerald-500 cursor-pointer"
                  >
                    {THAI_MONTHS.map((m, idx) => (
                      idx > 0 && <option key={idx} value={idx}>{m}</option>
                    ))}
                ...
                <div>
                  <label className="block text-[9px] font-semibold text-slate-500 uppercase">ปี พ.ศ.</label>
                  <select
                    value={selectedYearBE}
                    onChange={(e) => setSelectedYearBE(Number(e.target.value))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg py-1 px-2.5 text-xs font-semibold text-slate-700 focus:ring-1 focus:ring-emerald-500 cursor-pointer"
